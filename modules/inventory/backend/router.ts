...
// GET /api/inventory/export — download all products as CSV
router.get("/export", requireRole("member"), async (req, res, next) => {
  try {
    const tenantDb = req.tenantDb as any;
    const rows = await tenantDb.product.findMany({ orderBy: { createdAt: "desc" } });

    if (!rows.length) {
      res.setHeader("Content-Type", "text/csv");
      res.setHeader("Content-Disposition", `attachment; filename="products.csv"`);
      return res.send("id,name,sku,description,category,unitPrice,costPrice,unit,status,createdAt,updatedAt\n");
    }
...
// DELETE /api/inventory/bulk — delete multiple products by IDs
router.delete("/bulk", requireRole("manager"), async (req, res, next) => {
  try {
    ...
    const tenantDb = req.tenantDb as any;
    const { count } = await tenantDb.product.deleteMany({ where: { id: { in: ids } } });
