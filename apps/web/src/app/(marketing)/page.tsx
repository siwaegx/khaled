...
const features = [
  ...
  },
  {
    ...
    description: "Owners, managers, members. Granular permissions so everyone sees only what they need.",
  },
  {
    icon: BarChart3,
...
export default function LandingPage() {
  return (
    <>
      {/* ── Hero ─────────────────────────────────────────── */}
      <section className="relative overflow-hidden bg-hero-pattern bg-grid py-32 md:py-44 px-4 sm:px-6 text-center">
        <div className="pointer-events-none absolute -top-40 left-1/2 -translate-x-1/2 w-[1000px] h-[640px] rounded-full bg-primary/10 blur-3xl" />
        ...
        <div className="relative max-w-4xl mx-auto">
          <div className="inline-flex items-center gap-2 rounded-full border border-primary/25 bg-primary/8 px-4 py-1.5 text-sm font-medium text-primary mb-10 shadow-glow-sm">
          ...
          <h1 className="text-[3.5rem] sm:text-[4.5rem] md:text-[5.5rem] font-extrabold tracking-[-0.03em] leading-[1.04] mb-7">
            The modular ERP<br />
            built for{" "}
      ...
      {/* ── Stats bar ────────────────────────────────────── */}
      <section className="relative overflow-hidden bg-gradient-brand py-12 px-4 sm:px-6">
        <div className="pointer-events-none absolute inset-0 bg-grid opacity-10" />
        <div className="relative max-w-4xl mx-auto flex flex-col sm:flex-row items-center justify-around gap-10 sm:gap-4">
          {stats.map((s) => (
            <div key={s.label} className="text-center">
              <p className="text-5xl font-extrabold tracking-tight text-white font-heading">{s.value}</p>
              <p className="text-xs text-white/60 mt-2 font-medium tracking-widest uppercase">{s.label}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="features" className="py-28 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <Badge variant="secondary" className="mb-5 text-primary bg-primary/8 border-primary/20 font-medium tracking-wide">
              Why Business360
            </Badge>
            ...
            {steps.map((s) => (
              <div key={s.num} className="relative flex flex-col items-center text-center">
                <div className="relative w-12 h-12 rounded-full bg-gradient-brand flex items-center justify-center mb-5 shadow-glow-sm z-10">
                  <span className="text-white text-sm font-bold font-heading">{s.num}</span>
                </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2 mb-1">
                    ...
                    <span className={cn("text-xs font-semibold border rounded-full px-2 py-0.5", planColor)}>{plan}+</span>
                  </div>
      ...
      {/* ── Gradient CTA ─────────────────────────────────── */}
      <section className="relative overflow-hidden bg-gradient-brand py-32 px-4 sm:px-6">
        <div className="pointer-events-none absolute inset-0 bg-grid opacity-10" />
        <div className="pointer-events-none absolute -top-32 left-1/2 -translate-x-1/2 w-[700px] h-[500px] rounded-full bg-white/10 blur-3xl" />
        <div className="relative max-w-3xl mx-auto text-center">
          <h2 className="text-4xl md:text-[3.25rem] font-extrabold tracking-[-0.02em] leading-[1.1] text-white mb-6">
            Ready to run your<br />business smarter?
          </h2>
          <p className="text-white/70 text-lg mb-10 max-w-xl mx-auto leading-relaxed font-normal">
      ...
      <section className="py-12 px-4 sm:px-6 border-t bg-muted/25">
        <div className="max-w-5xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {[
            ...
            <div key={b} className="flex items-center gap-2.5 text-sm text-muted-foreground font-normal">
